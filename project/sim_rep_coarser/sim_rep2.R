...
ind_O <- (sapply(A_plot_id, f <- function(x)(any(x == hold_ls))) & 
            (grid.A[, 1] < 0.5))
...
ind_K <- (sapply(A_plot_id, f <- function(x)(any(x == hold_ls))) & 
            (grid.A[, 1] > 0.5))
...
# save output
output_filename <- paste0("./results/sim2/sim_", input_id, ".Rdata")
